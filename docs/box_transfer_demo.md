...
这是 **controlled-attachment software-in-the-loop** 演示。机器人根位姿与关节采用确定性关键帧；蓝箱保持为动态刚体，但控制过程明确分为三种模式：搬运时为 `GRIPPER`，每个 120 Hz 周期约束到双夹爪中点；夹爪完全张开后为 `TARGET HOLD`，箱体在桌角支撑位保持不动、双手先撤离；双手清空后切换为 `DYNAMIC`，停止约束并交还 PhysX。模式会逐帧写进时间线和合成视频，不能把该结果当成 MoveIt/ROS 2 闭环、力控或纯摩擦抓取验收。
...
| 15.25–16.50 s | `withdraw` | 切换为 `TARGET HOLD`；箱体保持在目标支撑位，双臂张开、底盘直线后退 |
| 16.50–17.75 s | `retreat` | 双手清空后切换为 `DYNAMIC`，再收臂并进入复位起点 |
...
受控搬运期间，箱体根节点不得低于桌面支撑面；下降末端在目标支撑高度钳制，不能用穿透桌面产生的 PhysX 排斥力“完成”放置。运输阶段箱底相对桌面至少抬高 `250 mm`。释放后额外运行一秒 PhysX 接触，再检查：
...
`manifest.json` 记录正式场景哈希、源码 bundle 哈希、Git 状态、Isaac Sim 版本、帧率、五路像素统计、完整 `acceptance_thresholds`、抓取门、附着/支撑高度误差、关节误差、放置误差和全部运行时检查。`ok: true` 只代表本文件定义的软件在环门槛通过。
