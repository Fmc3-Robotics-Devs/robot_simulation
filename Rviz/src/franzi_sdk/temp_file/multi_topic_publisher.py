...
_sdk_path = os.path.join(_project_root, "inbc_arm_sdk_py")
for path in [_current_dir, _sdk_path]:
    if path not in sys.path:
        sys.path.insert(0, path)

from inbc_arm_sdk_py.core3.client import ArmClient
...
class MultiTopicPublisher:
    ...
    def init_arm_client(self):
        ...
        if self.teleop_left_enabled:
            self.arm_client.SetEnableState(0, 1)
            time.sleep(0.1)
        ...
        if self.teleop_right_enabled:
            self.arm_client.SetEnableState(10, 1)
            time.sleep(0.1)
            self.arm_client.MoveJ(
                joint_path=[[0, 0, 0, math.pi/2, 0, 0, 0]],
                side=1,
