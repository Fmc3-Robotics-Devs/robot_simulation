...
void ovrVrInputStandard::RenderRunningFrame(
    ...
    for (int i = 0; i < (int)InputDevices.size(); ++i) {
        ...
        // 存入【世界坐标系】的手部矩阵（L/R 区分，与l/r对应）
        const char sideWorld = (side == 'l') ? 'L' : 'R'; // 大写标识世界坐标系
        handPoseTransformations.push_back(
            std::make_pair(sideWorld, handPoseMatrix));
        // 新增：头显矩阵处理（使用世界坐标或原始坐标）
        handPoseTransformations.push_back(
            std::make_pair('h', headPoseMatrix)); // 头显矩阵（标识'h'）
        ...
        // 新增：添加头显矩阵到容器
        TransformMatrices[axisSurfaces++] = headPoseMatrix; // 头显矩阵加入存储
