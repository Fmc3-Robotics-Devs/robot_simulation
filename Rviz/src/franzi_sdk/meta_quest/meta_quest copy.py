...
class MetaQuest:
    ...
    def get_left_arm_increment(self):
        ...
        return self._get_cartesian_increment("l", self.is_l_trig_pressed)

    def get_right_arm_increment(self):
        """右手柄增量（头显坐标系，RG按下时计算帧间增量）"""
        return self._get_cartesian_increment("r", self.is_r_trig_pressed)
    ...
    def get_left_arm_world_increment_R(self):
        """左手柄增量（世界坐标系，LG按下时计算帧间增量）"""
        return self._get_cartesian_increment_R("L", self.is_l_trig_pressed)

    def get_right_arm_world_increment_R(self):
        """右手柄增量（世界坐标系，RG按下时计算帧间增量）"""
        return self._get_cartesian_increment_R("R", self.is_r_trig_pressed)
