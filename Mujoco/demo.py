...
down, picks the part top-down with the left gripper, strafes 25 cm along the
bench carrying it, sets it down there and backs its arm out.
...
# Where the TCP sits relative to the part's top face: the palm's collision
# hull ends 28 mm above the TCP, so 25 mm down grips the part over ~24 mm of
# finger pad and keeps the palm 3 mm clear of it.
GRASP_DEPTH = 0.025
SIDE = "left"
...
def best_grasp(robot, position):
    """The square part can be taken at four yaws about the vertical; use the
    one whose IK stays furthest from the joint limits."""
    options = []
    for degrees in (0, 90, 180, -90):
        rotation = top_down(math.radians(degrees))
        solution, ok = robot.solve_ik(SIDE, position, rotation)
        if ok:
            options.append((robot.limit_margin(solution), rotation, solution))
    if not options:
        raise RuntimeError(f"no grasp orientation reaches {np.round(position, 3)}")
...
def pick_and_place(robot, callback=None):
    ...
    rotation, above = best_grasp(robot, grasp + [0, 0, APPROACH])
    robot.set_gripper(SIDE, 0.095)
    robot.move_joints(above, 2.5, callback)
    ...
    robot.step(0.5, callback)
    carried = np.linalg.norm(part.xpos - robot.tcp_pose(SIDE)[0])
    log(robot, f"strafed {STRAFE} m, part {carried * 1000:.1f} mm from the TCP")
    ...
    return dict(gap=gap, lifted=lifted, carried=carried, start=start, placed=placed,
